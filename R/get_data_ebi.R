...
get_data_ebi = function(accession, outdir){
  ...
  saveRDS(metadata, file = paste0(outdir, "/", accession, "_metadata.RDS"))
  ...
  for (i in 1:length(metadata$run_accession)){
    ...
    system(paste0("bash /scpackage/inst/get_data_ebi.sh -a ", acc, " -o ", outdir, " -l ", lib))
    system(paste0("mv ", acc, "* ", outdir))
    system(paste0("rm -rf /data2/ncbi_tmp/sra/", acc, "*"))
  }
  ########################################################################

}

#get_data_ebi(accession = "MGYS00000410",
#             outdir = "/data")
